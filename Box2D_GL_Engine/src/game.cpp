...
//Box2d Code
   ...
   //Box 2d y = 0 == GL =  y = 578
	...
	Line top_line(vec3(0,0,0), vec3(SCR_WIDTH, 0, 0));
	Line left_line(vec3(0,0,0), vec3(0, SCR_HEIGHT, 0));
	Line bottom_line(vec3(0,SCR_HEIGHT,0), vec3(SCR_WIDTH, SCR_HEIGHT, 0));
...
//				debug_line.draw();
			//	debug_line_2.draw();
				manager->Render();
				top_line.setMVP(o_cam->return_matrix());
				top_line.setColor(vec3(1.0f, 0.0f, 0.0f));
				top_line.draw();
				
				left_line.setMVP(o_cam->return_matrix());
				left_line.setColor(vec3(1.0f, 0.0f, 0.0f));
				left_line.draw();
			
				bottom_line.setMVP(o_cam->return_matrix());
				bottom_line.setColor(vec3(1.0f, 0.0f, 0.0f));
				bottom_line.draw();

				for(auto& line : manager->getLines())
				{
					line->setMVP(o_cam->return_matrix());
				}	

			std::cout << "PHYSICS ###############" << manager->Get("b2Chrono")->body->GetPosition().x << std::endl;
			manager->Get("b2Chrono")->body->SetLinearVelocity(b2Vec2( 0,manager->getWorld()->GetGravity().y));
			//manager->Get("b2Tori")->body->SetTransform(b2Vec2(200, 0), 0);
			//manager->Get("b2gengar")->body->SetTransform(b2Vec2(200, 0), 0);
			//manager->Get("b2Tori")->body->SetLinearVelocity(b2Vec2( x, y + manager->getWorld()->GetGravity().y  ));
			//manager->Get("b2Tori")->body->ApplyForce(b2Vec2(x, y), manager->Get("b2Tori")->body->GetWorldCenter(), true);
			manager->Get("b2Tori")->body->ApplyLinearImpulse(b2Vec2(x, y), manager->Get("b2Tori")->body->GetWorldCenter(), true);
